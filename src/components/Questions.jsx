...
export default function Questions() {
  ...
  const handleCancelSelection = (qIndex) => {
    const newSelectedAnswers = [...selectedAnswers];
    newSelectedAnswers[qIndex] = null; // Reset the selected answer for this question
    setSelectedAnswers(newSelectedAnswers);
  };
  ...
  const handleSubmitAnswer = (qIndex) => {
    ...
    if (selectedAnswer !== null) {
      const isCorrect = selectedAnswer === questions[qIndex].correctAnswer;
      const answerElement = document.querySelectorAll(".answer-option")[qIndex * 4 + selectedAnswer];
      ...
      <ul className="question-list">
        {questions.map((q, qIndex) => (
          <li key={qIndex} className="question-item">
            ...
            <button
              type="button"
              onClick={() => handleCancelSelection(qIndex)}
